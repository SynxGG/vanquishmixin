...
import vanquishmixin.init.VanquishmixinModScreens;
...
public class MenuStateUpdateMessage {
	...
	public static void handler(MenuStateUpdateMessage message, Supplier<NetworkEvent.Context> contextSupplier) {
		...
		context.enqueueWork(() -> {
			if (context.getSender().containerMenu instanceof VanquishmixinModMenus.MenuAccessor menu) {
				menu.getMenuState().put(message.elementType + ":" + message.name, message.elementState);
				if (!context.getDirection().getReceptionSide().isServer() && Minecraft.getInstance().screen instanceof VanquishmixinModScreens.ScreenAccessor accessor) {
	...
	public static void registerMessage(FMLCommonSetupEvent event) {
		VanquishmixinMod.addNetworkMessage(MenuStateUpdateMessage.class, MenuStateUpdateMessage::buffer, MenuStateUpdateMessage::new, MenuStateUpdateMessage::handler);
	}
}